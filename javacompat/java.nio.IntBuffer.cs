namespace java.nio
{
    public sealed class IntBuffer : global::java.nio.Buffer
    {
        ...
        private int ElemGet(int i) { return arr != null ? arr[i] : (int)VStore().ReadBytes(voff + i * 4, 4, vlittle); }
        private void ElemSet(int i, int v) { if (arr != null) { arr[i] = v; } else { VStore().WriteBytes(voff + i * 4, 4, v, vlittle); } }
